...
static int filter_case_find(uint32_t id);
static int filter_case_find_suite(uint32_t sid);

static void test_stat_add_failure(const struct UnitTest *test, uint32_t id);
static void test_stat_add_executed(const uint32_t cnt);
...
unsigned int gCaseId = 0;
unsigned int gSuiteId = 0;

static const char start_string[] = "[ RUN      ] (%02d_%03d_%04d) %s\n";
static const char pass_string[]  = "[       OK ] (%02d_%03d_%04d) %s\n";
static const char fail_string[]  = "[  FAILED  ] (%02d_%03d_%04d) %s\n";
static const char end_string[]   = "[==========] Running %d test(s).\n";

#define MAX_FAILURE_CNT 1024
static struct UnitTest global_failure_cases[MAX_FAILURE_CNT];
static uint32_t global_failure_cnt = 0;
static uint32_t global_total_case_cnt = 0;
...
int _run_test(
        ...
        const void* const heap_check_point, const uint32_t caseid) {
	...
	unsigned int module, suite, test;

	module = (caseid/10000)/1000;
	suite  = (caseid/10000)%1000;
	test   = (caseid%10000);
	...
	if (function_type == UNIT_TEST_FUNCTION_TYPE_TEST) {
		print_message(start_string, module, suite, test, function_name);
	}
	...
	if (setjmp(global_run_test_env) == 0) {
		...
		if (function_type == UNIT_TEST_FUNCTION_TYPE_TEST) {
			print_message(pass_string, module, suite, test, function_name);
		}
		rc = 0;
	} else {
		global_running_test = 0;
		print_message(fail_string, module, suite, test, function_name);
	}
...
#if 0
int _run_tests(const UnitTest * const tests, const size_t number_of_tests, const int id) {
	...
	const char** failed_names = malloc(number_of_tests *
	                                   sizeof(*failed_names));
	int* failed_ids = malloc(number_of_tests *
	                                   sizeof(*failed_ids));
	void **current_state = NULL;
	...
	while (current_test < number_of_tests) {
		...
		if (run_next_test) {
			int failed;
			uint32_t caseid = id*10000 + test->id;

			failed = _run_test(test->name, test->function, current_state,
			                       test->function_type, test_check_point, caseid);
			if (failed) {
				failed_names[total_failed] = test->name;
				failed_ids[total_failed] = caseid;
				failure_record_add(test, caseid);
			}
	...
	if (total_failed) {
		...
		for (i = 0; i < total_failed; i++) {
		    int module, suite, test;

            module = (failed_ids[i]/10000)/1000;
            suite  = (failed_ids[i]/10000)%1000;
            test   = (failed_ids[i]%10000);

			print_error(fail_string, module, suite, test, failed_names[i]);
		}
	} else {
		//print_error("\n %d FAILED TEST(S)\n", total_failed);
	}
...
#endif
int _run_tests_setup_teardown_id(
    const UnitTest * const tests, const size_t number_of_tests,
    UnitTestFunction setup, UnitTestFunction teardown, int id)
{
	int suite_setup_failed, suite_teardown_failed;
	// Check point of the heap state.
	const ListNode * const check_point = check_point_allocated_blocks();
	// Current test being executed.
	size_t current_test = 0;
	...
	/* A stack of test states.  A state is pushed on the stack
	 * when a test setup occurs and popped on tear down. */
	TestState* test_states = malloc(2 * sizeof(*test_states));
	size_t number_of_test_states = 0;
	// Names of the tests that failed.
	const char** failed_names = malloc(number_of_tests *
	                                   sizeof(*failed_names));
    ...
    *current_state = NULL;
    ...
    suite_setup_failed = _run_test("Suite_Setup", setup, current_state,
                           UNIT_TEST_FUNCTION_TYPE_SETUP, test_check_point, id);

	while (current_test < number_of_tests && suite_setup_failed == 0) {
		const UnitTest * const test = &tests[current_test++];
        int setup_failed, test_failed, teardown_failed;
        uint32_t caseid = id + test->id;

        if (!filter_case_find(caseid))
        {
            continue;
        }

		if (!test->function || !test->setup || !test->teardown) {
			continue;
		}
        ...
        *current_state = NULL;
        ...
        setup_failed = _run_test(test->name, test->setup, current_state,
                               UNIT_TEST_FUNCTION_TYPE_SETUP, test_check_point, caseid);

        //test
        if (!setup_failed)
        {
            test_failed = _run_test(test->name, test->function, current_state,
                               UNIT_TEST_FUNCTION_TYPE_TEST, test_check_point, caseid);
        }
        ...
        teardown_failed = _run_test(test->name, test->teardown, current_state,
                           UNIT_TEST_FUNCTION_TYPE_TEARDOWN, test_check_point, caseid);

        //update status
        if (setup_failed || test_failed || teardown_failed) {
            failed_names[total_failed] = test->name;
            failed_ids[total_failed] = caseid;
            test_stat_add_failure(test, caseid);
            total_failed ++;
        }
        tests_executed ++;
	}
    ...
    suite_teardown_failed = _run_test("Suite_Teardown", teardown, current_state,
                       UNIT_TEST_FUNCTION_TYPE_TEARDOWN, test_check_point, id);

    //suite report
    test_stat_add_executed(tests_executed);
    print_message("[==========] %d test(s) run.\n", tests_executed);
    print_error("[  PASSED  ] %d test(s).\n", tests_executed - total_failed);
	if (total_failed) {
		size_t i;
	...
	if (suite_setup_failed || suite_teardown_failed) {
        print_error("[  ERROR   ] Suite level failure setup(%d)/teardown(%d) "
                    , suite_setup_failed, suite_teardown_failed);
		total_failed = -1;
	}
...
void test_stat_init(void)
{
    global_failure_cnt = 0;
    global_total_case_cnt = 0;
    memset(global_failure_cases, 0, sizeof(global_failure_cases));
}

static void test_stat_add_failure(const struct UnitTest *test, uint32_t id)
{
    uint32_t i = global_failure_cnt;

    if (test && global_failure_cnt < MAX_FAILURE_CNT)
    {
        memcpy(&global_failure_cases[i], test, sizeof(*test));
        global_failure_cases[i].id = id;
        global_failure_cnt++;
    }
}
...
static void test_stat_add_executed(const uint32_t cnt)
{
    global_total_case_cnt += cnt;
}

void test_stat_report(void)
{
    uint32_t i, module, suite, test;
    struct UnitTest *t;

    fprintf(stderr, "\nTotal Case (%d), Failed (%d):\n", global_total_case_cnt, global_failure_cnt);
    for (i = 0; i < global_failure_cnt; i++)
    {
        t = &global_failure_cases[i];

        module = (t->id/10000)/1000;
        suite  = (t->id/10000)%1000;
        test   = (t->id%10000);

        fprintf(stderr, fail_string, module, suite, test, t->name);
    }
}

void default_setup(void **state)
{
    (void)state;
}

void default_teardown(void **state)
{
    (void)state;
}

void default_test(void **state)
{
    (void)state;
}
...
/// Case Filter Support
//
static const uint32_t *gpFilterCases = NULL;
static uint32_t        gFilterCaseCnt = 0;
void filter_case_setup(const uint32_t *cases, uint32_t cnt)
{
    gpFilterCases = cases;
    gFilterCaseCnt = cnt;
}
...
static int filter_case_find_suite(uint32_t sid)
{
    ...
    for (i = 0; i < gFilterCaseCnt; i++)
    {
        if (sid == *(gpFilterCases + i)/10000)
        {
            return 1;
        }
    }
